...
function FindAndLaunchCanvasJSGS() {
	...
	for ( var i = 0; i < canvases.length; i++ ) {
		...
		if (canvases[i].getAttribute) {
			if (canvases[i].getAttribute('data-jsgs'))
				launchfn = canvases[i].getAttribute('data-jsgs');
			if (canvases[i].getAttribute('data-fps'))
				launchfps = canvases[i].getAttribute('data-fps');
		} else {
			if (canvases[i].jsgs)
				launchfn = canvases[i]['data-jsgs'];
			if (canvases[i].fps)
				launchfps = canvases[i]['data-fps'];
		}
		if (launchfn) {
